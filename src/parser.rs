  ...
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      Expression::Call(s, args) => {
        ...
        for(i, arg) in args.iter().enumerate() { // write args
          if i > 0 { write!(f, ", ")?; }
          write!(f, "{}", arg)?;
        }
...
pub enum Statement {
  ...
  Function(String, Type, Option<String>, Option<Vec<Box<Statement>>>, Vec<Box<Statement>>),
  Struct(String, Vec<Box<Statement>>), Enum(String, Vec<String>),
  
  If(Expression, Vec<Box<Statement>>),
  Match(Expression, Vec<(Expression, Vec<Box<Statement>>)>, Option<Vec<Box<Statement>>>),
...
impl std::fmt::Display for Statement {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      Statement::Use(modname, namespace) => {
        match namespace {
...
impl Parser {
  ...
  fn parse_unary(&mut self) -> Result<Expression, ParserError> {
    match self.curr().get_value() {
      ...
      TokenValue::Identifier(id) => {
        self.parse_identifier_as_expression(id)
      }
  ...
  fn make_left_expr(&mut self, id: String) -> Result<Expression, ParserError> {
    match self.curr().get_value() {
      TokenValue::Dot => {
        ...
        Ok(Expression::Get(Box::new(Expression::Identifier(id)), Box::new(self.make_left_expr(field)?)))
      }
      ...
      TokenValue::OpenBracket => {
        ...
        self.next();
        Ok(Expression::Index(Box::new(Expression::Identifier(id)), Box::new(index)))
      }
  ...
  fn make_declaration(&mut self, id: String) -> Result<Statement, ParserError> {
    ...
    match self.curr().get_value() {
      ...
      TokenValue::Identifier(tid) => {
        if self.next().get_value() != TokenValue::Assign {
          return Ok(Statement::Declaration(id, Type::Custom(tid, is_const), None))
        }

        self.next();
        let expr = self.parse_expr()?;
        Ok(Statement::Declaration(id, Type::Custom(tid, is_const), Some(expr)))
      }
      _ => Err(ParserError::UnexpectedToken(self.curr()))
    }
  }
  ...
  fn make_defer(&mut self) -> Result<Statement, ParserError> {
    match self.next().get_value() {
      TokenValue::Identifier(id) => {
        self.next();
        Ok(Statement::Defer(Box::new(self.parse_identifier_as_statement(id)?)))
      }
  ...
  fn parse_identifier_as_expression(&mut self, id: String) -> Result<Expression, ParserError> {
    match self.next().get_value() {
      TokenValue::Dot => {
        ...
        Ok(Expression::Get(Box::new(Expression::Identifier(id)), Box::new(field)))
      }
    ...
    match self.curr().get_value() {
      ...
      TokenValue::OpenBracket => {
        ...
        if let TokenValue::Assign = self.curr().get_value() {
          ...
          return Ok(Statement::Assign(Expression::Index(Box::new(Expression::Identifier(id)), Box::new(index)), expr))
        }
