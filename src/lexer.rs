...
impl std::fmt::Display for Token {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "raw position: {} | line, column: {}, {} | token: {:?}", self.raw_pos, self.ln, self.col, self.value)
  }
}

struct Lexer<'a> {
  ln: usize, col: usize, pos: usize,
...
impl<'a> Lexer<'a> {
  fn new(src: &'a String) -> Self {
    ...
    Self { ln: 1, col: 0, pos: 0, src: chars, lookahead }
  }
  ...
  fn make_type(str: &String, is_const: bool) -> Option<Type> {
    match str.as_str() {
      ...
      "float" => Some(Type::Float(is_const)),
  ...
  fn tokenize(&mut self) -> Result<Vec<Token>, LexerError> {
    ...
    while self.lookahead.is_some() {
      ...
      match self.make_token() {
        Ok(tok) => {
          let tok = Token { value: tok, ln, col, raw_pos };
          tokens.push(tok);
        }
        Err(e) => {
          return Err(e)
    ...
    tokens.push(Token { value: TokenValue::Eof, ln: self.ln, col: self.col, raw_pos: self.pos });
