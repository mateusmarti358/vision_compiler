...
fn expression_to_c(expr: &Expression, symbol_table: &SymbolTable) -> String {
    match expr {
        ...
        Expression::Different(l, r) => format!(
            "{} != {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        Expression::LessThan(l, r) => format!(
            "{} < {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        Expression::GreaterThan(l, r) => format!(
            "{} > {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        Expression::LessEqual(l, r) => format!(
            "{} <= {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        Expression::GreaterEqual(l, r) => format!(
            "{} >= {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        ...
        Expression::Divide(l, r) => format!(
            "{} / {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        Expression::Mod(l, r) => format!(
            "{} % {}",
            expression_to_c(l, symbol_table),
            expression_to_c(r, symbol_table)
        ),
        ...
        Expression::Range(start, end) => {
            format!(
                "Range({}, {})",
                expression_to_c(start, symbol_table),
                expression_to_c(end, symbol_table)
            )
        }
...
fn statement_to_c(
    ...
    match stmt {
        Statement::Declaration(name, t, expr) => match expr {
            Some(expr) => (
                format!(
                    "{} {} = {};\n",
                    c_type(t, symbol_table),
                    name,
                    expression_to_c(expr, symbol_table)
                ),
                Some((name.clone(), t.clone())),
            ),
            None => (
                format!("{} {};\n", c_type(t, symbol_table), name),
                Some((name.clone(), t.clone())),
            ),
        },
        ...
        Statement::Assign(lexpr, rexpr) => (
            format!(
                "{} = {};\n",
                expression_to_c(lexpr, symbol_table),
                expression_to_c(rexpr, symbol_table)
            ),
            None,
        ),
        ...
        Statement::AddAssign(lexpr, rexpr) => (
            format!(
                "{} += {};\n",
                expression_to_c(lexpr, symbol_table),
                expression_to_c(rexpr, symbol_table)
        ...
        Statement::SubAssign(lexpr, rexpr) => (
            format!(
                "{} -= {};\n",
                expression_to_c(lexpr, symbol_table),
                expression_to_c(rexpr, symbol_table)
        ...
        Statement::MulAssign(lexpr, rexpr) => (
            format!(
                "{} *= {};\n",
                expression_to_c(lexpr, symbol_table),
                expression_to_c(rexpr, symbol_table)
        ...
        Statement::DivAssign(lexpr, rexpr) => (
            format!(
                "{} /= {};\n",
                expression_to_c(lexpr, symbol_table),
                expression_to_c(rexpr, symbol_table)
        ...
        Statement::ModAssign(lexpr, rexpr) => (
            format!(
                "{} %= {};\n",
                expression_to_c(lexpr, symbol_table),
                expression_to_c(rexpr, symbol_table)
            ...
            if let Expression::Get(l, r) = expr {
                ...
                loop {
                    match method_expression {
                        ...
                        Expression::Get(l, r) => {
                            self_arg = Expression::Get(
                                Box::new(self_arg.clone()),
                                Box::new((&**l).clone()),
                            );
                            method_expression = &**r
                        }
...
impl Transpiler {
    ...
    fn body(
        ...
        for stmt in body {
            match stmt.as_ref() {
                ...
                Statement::ForIn(var, iter, body) => {
                    ...
                    out.push_str(&format!(
                        "for ( auto {} : {} ) {{\n{}",
                        expression_to_c(var, &self.symbol_table),
                        expression_to_c(iter, &self.symbol_table),
                        self.body(
                            body,
                            varg,
                            [deferred.clone(), deferred_in_scope.clone()].concat()
                ...
                Statement::Defer(stmt) => deferred_in_scope.push(*stmt.clone()),
        ...
        for var in decl_curr_scope {
            self.symbol_table.unset_var(&var);
        }
    ...
    fn make_function(
        ...
        if name == "main" && super_name.is_none() {
        ...
        for default in defaults {
            ...
            body_str.push_str(&format!(
                "  {} = {};\n",
                default.0,
                expression_to_c(&default.2, &self.symbol_table)
            ));
            body_str.push_str("}\n");
        }
